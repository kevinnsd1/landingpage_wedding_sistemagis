...
export default function LandingPage() {
  ...
  const [selectedThemeCategory, setSelectedThemeCategory] =
  ...
  return (
    <div className="min-h-screen bg-[#FBF9F5] text-[#2A2421] font-sans flex flex-col justify-between relative overflow-hidden select-none">
      ...
      <div className="blush-gradient flex flex-col justify-between relative min-h-[480px] sm:min-h-screen">
        ...
        <main className="flex-1 flex items-center justify-center px-4 sm:px-8 relative my-auto py-6 sm:py-8 w-full max-w-7xl mx-auto overflow-hidden">
          ...
          <div className="w-full max-w-4xl mx-auto text-center relative z-10 px-6 sm:px-2 flex flex-col items-center justify-center overflow-visible">
            <div
              className={`w-full max-w-full transition-opacity duration-300 ease-out will-change-transform ${isFading ? "opacity-0" : "opacity-100"}`}
            >
            ...
            <div className="hidden sm:block absolute -top-2 sm:-top-3 right-4 md:right-12 opacity-80 hover:opacity-100 transition-opacity duration-300 pointer-events-none z-20">
              ...
              >
                ...
                />
                <path
                  d="M48 14L49.5 19L54.5 20.5L49.5 22L48 27L46.5 22L41.5 20.5L46.5 19L48 14Z"
                  fill="currentColor"
                  fillOpacity="0.8"
                />
                <path
                  d="M14 12L15 15L18 16L15 17L14 20L13 17L10 16L13 15L14 12Z"
                  fill="currentColor"
                  fillOpacity="0.6"
                />
              </svg>
      ...
      <section
        id="template"
        className="bg-[#FDFCF9] py-20 sm:py-28 border-t border-[#EBE5DA] relative"
      >
        <div className="max-w-6xl mx-auto px-6 sm:px-12 space-y-16 sm:space-y-24">
          ...
          <div className="text-center space-y-4 max-w-2xl mx-auto">
            ...
            <p className="text-xs sm:text-sm text-[#78736A] leading-relaxed">
              Tersedia beragam tema desain pernikahan minimalis &amp; elegan
              yang dapat disesuaikan secara instan.
            </p>
            ...
            <div className="pt-2">
              ...
              >
                <LayoutGrid className="w-4 h-4 text-amber-300" /> Lihat Semua
                Tema (Katalog Galeri)
              </button>
            </div>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="w-full md:w-1/2 flex justify-center">
                    <div
                      onClick={() => {
                        if (item.id === "template-1") {
                          window.location.href = "/demo/theme-1";
                        } else {
                          setIsThemeGalleryOpen(true);
                        }
                      }}
                      className={`w-full max-w-md aspect-4/3 ${item.bgClass || "bg-gradient-to-br from-[#DCEEFE] via-[#B9E0FE] to-[#8ECDFF]"} rounded-2xl p-6 flex flex-col items-center justify-center border border-sky-300/40 shadow-lg hover:shadow-2xl hover:scale-[1.02] transition-all duration-300 group cursor-pointer relative overflow-hidden`}
                    >
                      {/* Subtle ambient background glows */}
                      <div className="absolute -top-12 -right-12 w-48 h-48 rounded-full bg-white/35 blur-2xl pointer-events-none" />
                      <div className="absolute -bottom-12 -left-12 w-48 h-48 rounded-full bg-white/25 blur-2xl pointer-events-none" />

                      {/* Main Image Asset directly on full blue background */}
                      <div className="relative w-full h-full flex items-center justify-center p-2">
                        {item.imageSrc ? (
                          <img
                            src={item.imageSrc}
                            alt={item.title}
                            className="max-h-[220px] w-auto object-contain filter drop-shadow-xl group-hover:scale-105 transition-transform duration-500"
                          />
                        ) : (
                          <ImageIcon className="h-10 w-10 text-sky-700" />
                        )}
                      </div>
                    </div>
                  </div>
          ...
          <div className="bg-[#F8F5EE] rounded-3xl p-8 sm:p-10 border border-[#EBE5DA] text-center space-y-4 max-w-3xl mx-auto card-smooth-hover shadow-sm">
            ...
            <p className="text-xs sm:text-sm text-[#78736A] max-w-md mx-auto font-light">
              Jelajahi seluruh koleksi tema warna Botanical, Minimalist, Luxury,
              Rustic, hingga Adat Tradisional di galeri interaktif kami.
            </p>
            <div>
              <button
                onClick={() => setIsThemeGalleryOpen(true)}
              ...
              >
                <Palette className="w-4 h-4 text-amber-300" /> Buka Katalog
                Galeri Tema
              </button>
            </div>
          </div>
      ...
      <footer className="bg-[#FBF9F5] border-t border-[#EBE5DA] py-8 text-center text-xs text-[#78736A]">
        <div className="max-w-6xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <p>© 2026 KisahMagis. Undangan Digital Wedding Minimalis &amp; Eksklusif.</p>
        </div>
      </footer>
                ...
                {filteredThemes.map((theme) => (
                  ...
                  >
                    <div>
                      ...
                      <div
                        className={`w-full aspect-16/9 ${theme.previewBg} rounded-xl border ${theme.accentColor} p-4 flex flex-col justify-between shadow-xs mb-4 relative overflow-hidden group-hover:scale-[1.02] transition-transform`}
                      >
      ...
      {activePreviewTheme && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 bg-black/75 backdrop-blur-md overflow-y-auto animate-in zoom-in-95 duration-200">
          <div className="bg-[#FBF9F5] rounded-3xl max-w-2xl w-full border border-[#EBE5DA] shadow-2xl p-6 sm:p-8 space-y-6 relative">
            ...
            <div
              className={`w-full aspect-16/10 ${activePreviewTheme.previewBg} rounded-2xl border-2 ${activePreviewTheme.accentColor} p-6 flex flex-col justify-between shadow-inner text-center relative overflow-hidden space-y-4`}
            >
              <div className="flex items-center justify-between text-xs font-mono">
                <span className="uppercase tracking-widest font-semibold">
                  {activePreviewTheme.category}
                </span>
                <span className="bg-white/80 px-2 py-0.5 rounded text-[#2A2421] font-bold">
                  {activePreviewTheme.badge}
                </span>
              </div>
              ...
              <div className="flex items-center justify-center gap-2 pt-2">
                <span className="text-[10px] font-mono uppercase text-[#78736A]">
                  Palet Warna:
                </span>
